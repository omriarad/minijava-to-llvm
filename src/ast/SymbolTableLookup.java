...
public class SymbolTableLookup {
  ...
  SymbolTable lookup(String curClass, String curMethod, String type, String name) {
    ...
    while (!curSymbolTableEntries.containsKey(name)
        || !curSymbolTableEntries.get(name).getType().equals(type)) {
  ...
  boolean isAncestorSus(
      String curClass, String methodName, Set<String> susClasses, String susClass) {
    Map<String, SymbolTable> scopeToSymbolTable = this.classToScopes.get(curClass);
    if (scopeToSymbolTable == null) {
      return false;
    }
    ...
    while (curSymbolTable != null) {
      if (susClasses.contains(curSymbolTable.getScopeName())) {
        SymbolTable result = this.lookup(curSymbolTable.getScopeName(), null, "method", methodName);
        if (result != null) {
          return true;
        }
  ...
  private void initFieldToOffsets() {
    for (Map.Entry<String, Map<String, SymbolTable>> entry : classToScopes.entrySet()) {
      ...
      if (classSymbolTable.getParentSymbolTable() != null) {
        ...
        FieldOffsetsList parentList =
            classToFieldOffsets.get(classSymbolTable.getParentSymbolTable().getScopeName());
  ...
  private void initVTable() {
    for (Map.Entry<String, Map<String, SymbolTable>> entry : classToScopes.entrySet()) {
      ...
      if (classSymbolTable.getParentSymbolTable() != null) {
        ...
        VTable parentList =
            classToVTable.get(classSymbolTable.getParentSymbolTable().getScopeName());
