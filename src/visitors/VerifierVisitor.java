...
public class VerifierVisitor implements Visitor {
  ...
  class VerificationError extends AssertionError {
    public VerificationError(String error) {
      super(
          String.format(
              "<%s:%s> %s",
              VerifierVisitor.this.currentClass, VerifierVisitor.this.currentMethod, error));
    }
  }
    ...
    if (originalReturnType.compareTo(overridingReturnType) != 0
      ...
      throw new VerificationError(
          String.format(
              ...
              originalReturnType, overridingReturnType));
    }
  }
  ...
  public void visit(MethodDecl methodDecl) {
    ...
    if (this.type.compareTo(returnType) != 0
      ...
      throw new VerificationError(
          String.format("invalid return type, static=%s got=%s", returnType, this.type));
    }
  ...
  public void visit(IfStatement ifStatement) {
    ...
    if (this.type.compareTo("boolean") != 0) {
      throw new VerificationError(
          String.format("cond in if must be boolean and not %s", this.type));
    }
  }
  ...
  public void visit(WhileStatement whileStatement) {
    ...
    if (this.type.compareTo("boolean") != 0) {
      throw new VerificationError(
          String.format("cond in while must be boolean and not %s", this.type));
    }
  ...
  public void visit(AssignStatement assignStatement) {
    ...
    if (lType.compareTo(rType) != 0 && !this.symbolTables.isSubclass(lType, rType)) {
      throw new VerificationError(
          String.format("type mismatch in assignment, rType=%s lType=%s", rType, lType));
    }
  }
  ...
  public void visit(MethodCallExpr e) {
    ...
    if (method == null) {
      throw new VerificationError(
          String.format("method %s not found for class %s", e.methodId(), this.type));
    }
    ...
    for (int i = 0; i < methodArgCount; i++) {
      ...
      if (this.type.compareTo(methodType) != 0
        ...
        throw new VerificationError(
            String.format("type of formal %d mismatch for method %s", i, e.methodId()));
      }
    }
  ...
  public void visit(NewObjectExpr e) {
    if (!this.symbolTables.isDefined(e.classId())) {
      throw new VerificationError(
          String.format("refType %s doesn't exists in symbol tables", e.classId()));
    }
  ...
  public void visit(RefType t) {
    if (!this.symbolTables.isDefined(t.id())) {
      throw new VerificationError(
          String.format("refType %s doesn't exists in symbol tables", t.id()));
    }
