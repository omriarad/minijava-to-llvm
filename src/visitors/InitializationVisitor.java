...
public class InitializationVisitor implements Visitor {
  ...
  public InitializationVisitor(Map<String, Map<String, SymbolTable>> symbolTables) {
  ...
  private HashMap<String, String> laticeMapUnion(
      HashMap<String, String> latticeMap1, HashMap<String, String> latticeMap2) {
    HashMap<String, String> unifiedLaticeMap = new HashMap<>();
    for (Map.Entry<String, String> entry : latticeMap1.entrySet()) {
      if (entry.getValue().equals("T")
          || (!entry.getValue().equals(latticeMap2.get(entry.getKey())))) {
    ...
    if (!this.curLatticeMap.get(assignArrayStatement.lv()).equals("tt")) {
      throwCompilationError(
          assignArrayStatement.lv()
              + " is used before initialized (class: "
              + this.currentClass
              + " method: "
              + this.currentMethod
              + ")");
